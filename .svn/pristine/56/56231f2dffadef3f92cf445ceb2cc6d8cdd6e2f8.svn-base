//
//  NNTransferTradingViewController.m
//  YWL
//
...
@interface NNTransferTradingViewController ()
...
@implementation NNTransferTradingViewController
...
{
    self.ensureButton.enabled = self.countField.hasText && self.codeTextFieldField.text.length == 6;
}
...
- (void)inputPayCode:(NSString *)paycode
{
    ...
    NNAPIBlockFundTransferTool *blockTool = [[NNAPIBlockFundTransferTool alloc] initWalletTransferWithTransferType:self.currentTransferType transferNum:self.countField.text account:@"" verificationCode:self.codeTextFieldField.text paypwd:paycode];
...
{
    if (_codeButton == nil) {
        NNHSendVerificationCodeType codeType = _currentTransferType == NNHWalletTransferTypeLTTrading ? NNHSendVerificationCodeType_LTTrading : NNHSendVerificationCodeType_TradingLT;
        _codeButton = [[NNCountDownButton alloc] initWithTotalTime:60 titleBefre:@"获取验证码" titleConting:@"s" titleAfterCounting:@"获取验证码" clickAction:^(NNCountDownButton *countBtn) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                NNHApiSecurityTool *tool = [[NNHApiSecurityTool alloc] initWithMobile:[NNHProjectControlCenter sharedControlCenter].userControl_currentUserModel.completemobile verifyCodeType:codeType countryCode:nil];
