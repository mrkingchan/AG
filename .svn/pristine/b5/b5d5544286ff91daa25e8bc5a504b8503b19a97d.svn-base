...
{
    if (self.dataSource.count == 0) [self.addButton setTitle:@"添加" forState:UIControlStateNormal];
    self.manageButton.hidden = self.dataSource.count == 0;
    if (self.dataSource.count == 0) {
        self.manageButton.selected = NO;
        [self.addButton setTitle:@"添加" forState:UIControlStateNormal];
    }
...
- (UIButton *)manageButton
{
    if (_manageButton == nil) {
        ...
        [_manageButton setTitleColor:[UIConfigManager colorThemeDarkGray] forState:UIControlStateNormal];
        _manageButton.titleLabel.font = [UIConfigManager fontThemeTextMain];
...
{
    if (_addButton == nil) {
        _addButton = [UIButton NNHOperationBtnWithTitle:@"添加" target:self action:@selector(clickAddButton) operationButtonType:NNHOperationButtonTypeBlue isAddCornerRadius:NO];
        [_addButton setBackgroundColor:[UIColor akext_colorWithHex:@"#d1d1da"] forState:UIControlStateNormal];
        [_addButton setTitleColor:[UIConfigManager colorThemeRed] forState:UIControlStateNormal];
    }
    return _addButton;
}
