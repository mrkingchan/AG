...
- (void)setupTitleButtons {
    ...
    [self.titleArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        CGFloat tempWidth = [self nnh_widthWithString:obj font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]];
    ...
    if (self.allBtnWidth <= self.bounds.size.width + SGPageTitleViewBtnMargin) { /// SGPageTitleView 不可滚动
        ...
        for (NSInteger index = 0; index < self.titleArr.count; index++) {
            UIButton *btn = [UIButton buttonWithType:(UIButtonTypeCustom)];
            // 设置 frame
            CGFloat btnW = [self nnh_widthWithString:self.titleArr[index] font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]] + SGPageTitleViewBtnMargin;
            btn.frame = CGRectMake(btnX, btnY, btnW, btnH);
            btnX = btnX + btnW;
            btn.tag = index;
            btn.titleLabel.font = [UIConfigManager fontThemeTextDefault];
        ...
        if (self.indicatorAnimation) {
            [UIView animateWithDuration:SGIndicatorAnimationTime animations:^{
                if (self.indicatorStyle == NNHIndicatorTypeEqual) {
                    self.indicatorView.nnh_width = [self nnh_widthWithString:button.currentTitle font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]];
...
- (void)changeSelectedButton:(UIButton *)button {
    if (self.tempBtn == nil) {
        button.titleLabel.font = [UIConfigManager fontThemeTextDefault];
        button.selected = YES;
        self.tempBtn = button;
        ...
        self.tempBtn.titleLabel.font = [UIConfigManager fontThemeTextDefault];
        button.selected = YES;
        self.tempBtn = button;
        button.titleLabel.font = [UIConfigManager fontThemeTextDefault];
    }
...
- (void)setPageTitleViewWithProgress:(CGFloat)progress originalIndex:(NSInteger)originalIndex targetIndex:(NSInteger)targetIndex {
    ...
    if (self.allBtnWidth <= self.bounds.size.width) { /// SGPageTitleView 不可滚动
        if (self.isIndicatorScroll) {
            ...
            CGFloat targetBtnX = CGRectGetMaxX(targetBtn.frame) - 0.5 * (self.nnh_width / self.titleArr.count - [self nnh_widthWithString:targetBtn.currentTitle font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]]) - [self nnh_widthWithString:targetBtn.currentTitle font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]];
            CGFloat originalBtnX = CGRectGetMaxX(originalBtn.frame) - 0.5 * (self.nnh_width / self.titleArr.count - [self nnh_widthWithString:originalBtn.currentTitle font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]]) - [self nnh_widthWithString:originalBtn.currentTitle font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]];
            CGFloat totalOffsetX = targetBtnX - originalBtnX;
            /// 计算 indicatorView 滚动时 X 的偏移量
            CGFloat offsetX;
            /// 计算 targetBtn／originalBtn 宽度的差值
            CGFloat targetBtnDistance = (CGRectGetMaxX(targetBtn.frame) - 0.5 * (self.nnh_width / self.titleArr.count - [self nnh_widthWithString:targetBtn.currentTitle font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]]));
            CGFloat originalBtnDistance = (CGRectGetMaxX(originalBtn.frame) - 0.5 * (self.nnh_width / self.titleArr.count - [self nnh_widthWithString:originalBtn.currentTitle font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]]));
            ...
            if (self.indicatorStyle == NNHIndicatorTypeEqual) {
                ...
                temp.size.width = [self nnh_widthWithString:originalBtn.currentTitle font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]] + distance;
            ...
            if (progress > 0.5) {
                if (self.indicatorAnimation) {
                    [UIView animateWithDuration:SGIndicatorAnimationTime animations:^{
                        if (self.indicatorStyle == NNHIndicatorTypeEqual) {
                            self.indicatorView.nnh_width = [self nnh_widthWithString:targetBtn.currentTitle font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]];
...
- (void)setIndicatorStyle:(NNHIndicatorType)indicatorStyle {
    ...
    if (indicatorStyle == NNHIndicatorTypeEqual) {
        if (self.selectedIndex) {
            ...
            self.indicatorView.nnh_width = [self nnh_widthWithString:self.titleArr[self.selectedIndex] font:[UIFont systemFontOfSize:SGPageTitleViewTextFont]];
