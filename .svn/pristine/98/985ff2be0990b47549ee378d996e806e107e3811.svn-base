...
- (void)setupChildView
{
    ...
    self.backgroundView.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT * 0.8);
...
- (void)showInFatherView:(UIView *)fatherView
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.backgroundView.nnh_y = SCREEN_HEIGHT * 0.2;
    } completion:^(BOOL finished) {
        [self resetStatus];
    }];

    //发送通知启用导航栏右滑返回
    [[NSNotificationCenter defaultCenter] postNotificationName:NNH_NotificationPayView_dispalyPayView object:nil];
}
...
- (void)dissmissWithCompletion:(void (^)(void))completion
{
    ...
    //发送通知启用导航栏右滑返回
    [[NSNotificationCenter defaultCenter] postNotificationName:NNH_NotificationPayView_hiddenPayView object:nil];
...
- (void)missPayCode
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [[NNHApplicationHelper sharedInstance] updatePayPassword];
    });
}
