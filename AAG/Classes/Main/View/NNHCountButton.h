...
@protocol NNHCountButtonDelegate <NSObject>

/**
 点击加减代理回调

 @param countButton 数量按钮
 @param count 当前数量
 @param increaseStatus 是否为加的状态
 */
- (void)nnh_countButton:(__kindof UIView *)countButton count:(NSInteger)count increaseStatus:(BOOL)increaseStatus;

@end


IB_DESIGNABLE
@interface NNHCountButton : UIView
...
/** 加减按钮的Block回调*/
@property (nonatomic, copy) void(^resultBlock)(NSInteger count, BOOL increaseStatus/* 是否为加状态*/);

/** 代理对象 */
@property (nonatomic, weak) id<NNHCountButtonDelegate> delegate;
...
@interface NSString (NNHCountButton)
/**
 字符串 nil, @"", @"  ", @"\n" Returns NO;
 其他 Returns YES.
 */
