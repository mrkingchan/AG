...
- (void)setupChildView
{
    ...
    [self.mainImage mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.contentView).insets(UIEdgeInsetsMake(NNHMargin_10, NNHMargin_10, NNHMargin_10, NNHMargin_10));
    }];
    
    [self.contentView addSubview:self.deleteButton];
    [self.deleteButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.top.equalTo(self.contentView);
        make.size.mas_equalTo(CGSizeMake(NNHMargin_25, NNHMargin_25));
    }];
...
- (UIButton *)deleteButton
{
    if (_deleteButton == nil) {
        _deleteButton = [UIButton NNHBtnImage:@"ic_delete_picture" target:self action:@selector(deleteButtonClick:)];
    }
