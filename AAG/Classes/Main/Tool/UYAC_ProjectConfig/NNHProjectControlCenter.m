...
//#import "NNHLoginRegisterViewController.h"
...
- (BOOL)loginStatus:(BOOL)isJumpLogin
{
    if (![self.userControl isLoginIn]){
        if (isJumpLogin) {
//            [NNHLoginRegisterViewController presentInViewController:[UIApplication sharedApplication].keyWindow.rootViewController withAniamtion:YES andCompletion:nil];
        }
        return NO;
    }else{
        return YES;
...
- (BOOL)loginStatus:(BOOL)isJumpLogin complete:(void(^)(void))complete
{
    if (![self.userControl isLoginIn]){
        if (isJumpLogin) {
//            [NNHLoginRegisterViewController presentInViewController:[UIApplication sharedApplication].keyWindow.rootViewController withAniamtion:YES andCompletion:^{
//                complete();
//            }];
        }
        return NO;
