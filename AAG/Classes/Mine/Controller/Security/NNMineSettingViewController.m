...
- (void)setupGroup0
{
    ...
    if ([userModel.isnameauth isEqualToString:@"1"]) {
        realItem.rightTitle = @"已认证";
    }else if ([userModel.isnameauth isEqualToString:@"0"]) {
        realItem.rightTitle = @"未认证";
    }else {
        realItem.rightTitle = @"审核中";
    }
    if (![userModel.isnameauth isEqualToString:@"2"]) {
